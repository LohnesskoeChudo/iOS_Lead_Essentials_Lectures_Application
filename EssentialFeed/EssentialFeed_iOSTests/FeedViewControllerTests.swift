...
final class FeedViewControllerTests: XCTestCase {
    ...
    func test_loadImageDataRetryButton_isVisibleOnImageDataLoadingError() {
        ...
        loader.completeImageDataLoadingWithSuccess(data: imageData, at: 0)
        XCTAssertEqual(view0?.isShowingRetryButton, false)
        XCTAssertEqual(view1?.isShowingRetryButton, false)
    ...
    func test_loadImageDataRetryButton_isVisibleOnInvalidImageData() {
        let image = makeImage(url: URL(string: "http://image-url-0.com")!)
        let (loader, sut) = makeSut()
        
        sut.loadViewIfNeeded()
        loader.completeFeedLoadingWith(feed: [image])
        let view0 = sut.simulateFeedImageViewBecomeVisible(at: 0)
        
        XCTAssertEqual(view0?.isShowingRetryButton, false)
        
        let invalidData = Data("invalid data".utf8)
        loader.completeImageDataLoadingWithSuccess(data: invalidData, at: 0)
    ...
    func test_feedImageView_cancelsPreloadingWhenNotVisibleAnymore() {
        let image0 = makeImage(url: URL(string: "http://image-url-0.com")!)
        let image1 = makeImage(url: URL(string: "http://image-url-1.com")!)
        let (loader, sut) = makeSut()
    ...
    private func makeSut(file: StaticString = #filePath, line: UInt = #line) -> (SpyLoader, FeedViewController) {
        ...
        let sut = FeedUIComposer.make(feedLoader: loader, imageDataLoader: loader)
...
private extension FeedViewController {
    ...
    func simulateFeedImageViewNotNearVisible(at index: Int) {
        simulateFeedImageViewBecomeVisible(at: index)
        
        let dataSourse = tableView.prefetchDataSource
        let indexPath = IndexPath(row: index, section: feedSection)
        dataSourse?.tableView?(tableView, cancelPrefetchingForRowsAt: [indexPath])
    }
